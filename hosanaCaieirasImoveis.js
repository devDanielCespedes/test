// IGUAL AO VIVA REAL e ao lopes e zap
...
const hosanaCaieirasImoveisImgSubDirs = [
  "340w",
  "768w",
  "1080w",
  "200x200",
...
hosanaCaieirasImoveisImgSubDirs.forEach((subDir) => {
...
const hosanaCaieirasImoveisWebPagesDir = path.join(
  __dirname,
  "hosanaCaieirasImoveisWebPages"
);
if (!fs.existsSync(hosanaCaieirasImoveisWebPagesDir)) {
  fs.mkdirSync(hosanaCaieirasImoveisWebPagesDir);
}
...
(async () => {
  ...
  const pageUrl =
    "https://www.hosanaimoveis.com.br/imovel/sobrado-2-quartos-alpes-de-caieiras-caieiras-8-vagas-1400m2-code-3064";
  ...
  const downloadPromises = resourceUrls.map(async (resourceUrl) => {
    try {
      ...
      if (
        ...
        hosanaCaieirasImoveisImgSubDirs.forEach(async (subDir) => {
  ...
  const regex = /https:\/\/www\.hosanaimoveis\.com\.br\/imovel\/.*-code-(\d+)/;
  ...
  const outputPath = path.join(
    hosanaCaieirasImoveisWebPagesDir,
