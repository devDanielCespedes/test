// QUASE FUNCIONANDO, PRECISA TIRAR OS NUMEROS DOS ARQUIVOS CSS QUE SÃO SALVOS PARA FUNCIONAR PARTE DO CSS
// PRECISA ARRUMAR AS IMAGENS QUE NÃO ESTÃO APARECENDO E ENTENDER O QUE FEZ SUMIR O HEADER E A BARRA
// DE NAVEGAÇÃO
...
sandraImoveisImgSubDirs.forEach((subDir) => {
...
const sandraImoveisWebPagesDir = path.join(__dirname, "sandraImoveisWebPages");
if (!fs.existsSync(sandraImoveisWebPagesDir)) {
  fs.mkdirSync(sandraImoveisWebPagesDir);
}

const cssDir = path.join(__dirname, "sandraImoveisWebPages", "css");
...
(async () => {
  ...
  const pageUrl =
    "https://www.sandraimoveis.com.br/462_imovel_ribeirao_preto_imobiliaria_Jardim_Palma_Travassos_Apartamento";
  ...
  const downloadPromises = resourceUrls.map(async (resourceUrl) => {
    try {
      ...
      if (
        ...
        sandraImoveisImgSubDirs.forEach(async (subDir) => {
          ...
          resourceMapping[parsedUrl.href] = `./img/${subDir}/${fileName}`;
        });
      } else if (fileName.endsWith(".css")) {
        const destination = path.join(cssDir, fileName);
        await downloadResource(parsedUrl.href, destination);
        resourceMapping[parsedUrl.href] = `./css/${fileName}`;
      }
    } catch (error) {
      console.error(`Failed to download ${resourceUrl}: ${error.message}`);
    }
  ...
  const regex = /https:\/\/www\.sandraimoveis\.com\.br\/(\d+)_imovel/;
