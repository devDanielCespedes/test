// Não deu certo. Falta arrumar o css
...
yellowImoveisImgSubDirs.forEach((subDir) => {
...
const yellowImoveisWebPagesDir = path.join(__dirname, "yellowImoveisWebPages");
if (!fs.existsSync(yellowImoveisWebPagesDir)) {
  fs.mkdirSync(yellowImoveisWebPagesDir);
}
...
(async () => {
  ...
  const pageUrl =
    "https://www.yellowimoveis.com.br/imovel/galpao-lagoa-aracariguama-1684,66m2-code-8407";
  ...
  const downloadPromises = resourceUrls.map(async (resourceUrl) => {
    try {
      ...
      if (
        ...
        yellowImoveisImgSubDirs.forEach(async (subDir) => {
  ...
  const regex = /https:\/\/www\.yellowimoveis\.com\.br\/imovel\/.*-code-(\d+)/;
