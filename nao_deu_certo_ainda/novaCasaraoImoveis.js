...
const novaCasaraoImoveisImgSubDirs = [
  "340w",
  "768w",
  "1080w",
  "200x200",
...
novaCasaraoImoveisImgSubDirs.forEach((subDir) => {
...
const novaCasaraoImoveisWebPagesDir = path.join(
  __dirname,
  "novaCasaraoImoveisWebPages"
);
if (!fs.existsSync(novaCasaraoImoveisWebPagesDir)) {
  fs.mkdirSync(novaCasaraoImoveisWebPagesDir);
}
...
(async () => {
  ...
  const pageUrl =
    "https://www.novacasarao.com.br/imovel/casa-praia-grande-2-quartos-60-m/CA4205-NOW9?from=sale";
  ...
  const downloadPromises = resourceUrls.map(async (resourceUrl) => {
    try {
      ...
      novaCasaraoImoveisImgSubDirs.forEach(async (subDir) => {
  ...
  const regex =
    /https:\/\/www\.novacasarao\.com\.br\/imovel\/.*\/(CA\d+-NOW\d+)/;
