// IGUAL AO VIVA REAL e ao zap
// precisa investigar pq o logo não está sendo setado
...
const lopesImoveisImgSubDirs = ["REO835156"];

lopesImoveisImgSubDirs.forEach((subDir) => {
...
const lopesImoveisWebPagesDir = path.join(__dirname, "lopesImoveisWebPages");
if (!fs.existsSync(lopesImoveisWebPagesDir)) {
  fs.mkdirSync(lopesImoveisWebPagesDir);
}
...
(async () => {
  ...
  const pageUrl =
    "https://www.lopes.com.br/imovel/REO835156/venda-apartamento-4-quartos-sao-paulo-jardim-paulista?listFrom=busca&listPosition=1";
  ...
  const downloadPromises = resourceUrls.map(async (resourceUrl) => {
    try {
      ...
      if (
        ...
        lopesImoveisImgSubDirs.forEach(async (subDir) => {
  ...
  const regex = /https:\/\/www\.lopes\.com\.br\/imovel\/([A-Z]{3}\d+)/;
